...
{
    float degree = pi*p;
    return 1.0/cos(degree) *mat2(vec2(1, 0),
                                 vec2(0, cos(degree)));
}
...
vec2 transitionCoord(vec2 coordinate, mat2 matrix, float offset, float p)
{
    ...
    vec2 coor = matrix * vec2(center.x + sin(pi*p)*offset, center.y);
...
vec4 transition (vec2 uv) {
    ...
    vec2 greenCoord = transitionCoord(uv, matrix, 0.0, progress);
    ...
    if (!inBounds(redCoord)) {
      redPlane = black;
        ...
        redPlane = vec4(src.r, 0, 0, src.a);
    } else {
        vec4 des = getToColor(vec2(1.0-redCoord.x, redCoord.y));
    ...
    if (!inBounds(greenCoord)) {
        ...
        vec4 des = getToColor(vec2(1.0-greenCoord.x, greenCoord.y));
    ...
    if (!inBounds(blueCoord)) {
        ...
        bluePlane = vec4(0.0, 0.0, src.b, src.a);
    } else {
        vec4 des = getToColor(vec2(1.0-blueCoord.x, blueCoord.y));;
        bluePlane = vec4(0.0, 0.0, des.b, des.a);
    }
